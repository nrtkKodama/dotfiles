...
*   `git` (managed by the setup script, but good to have)
...
This project supports a configuration-based setup using `config.env`. This allows you to customize the installation (e.g., username, repository URL) without modifying the source code.
...
### 🖥️ `Ubuntu` [![Ubuntu](https://github.com/nrtkKodama/dotfiles/actions/workflows/ubuntu.yaml/badge.svg)](https://github.com/nrtkKodama/dotfiles/actions/workflows/ubuntu.yaml)
...
*Note: If `wget` is not available, you can use `curl`:*
```console
bash -c "$(curl -fsLS https://raw.githubusercontent.com/nrtkKodama/dotfiles/main/setup.sh)"
```
...
Each installation script can be found under the [`./install`](https://github.com/nrtkKodama/dotfiles/tree/master/install) directory.
...
The setup scripts are stored as shellscripts in an appropriate location under the [`./install`](https://github.com/nrtkKodama/dotfiles/tree/master/install) directory.
After verifying that the shellscript works, store the [chezmoi template](https://www.chezmoi.io/user-guide/templating/)-based file, which is based on the shellscript, in an appropriate location under the [`./home/.chezmoiscripts`](https://github.com/nrtkKodama/dotfiles/tree/master/home/.chezmoiscripts) directory.

Below is the correspondence between shellscript and template for docker installation on MacOS.

- The shellscript for docker: [`install/macos/common/docker.sh`](https://github.com/nrtkKodama/dotfiles/blob/master/install/macos/common/docker.sh)
- The chezmoi template for docker: [`home/.chezmoiscripts/macos/run_once_10-install-docker.sh.tmpl`](https://github.com/nrtkKodama/dotfiles/blob/master/home/.chezmoiscripts/macos/run_once_10-install-docker.sh.tmpl)
...
### 🦇 Unit Test with [Bats](https://github.com/bats-core/bats-core) [![Unit test](https://github.com/nrtkKodama/dotfiles/actions/workflows/test.yaml/badge.svg)](https://github.com/nrtkKodama/dotfiles/actions/workflows/test.yaml)

Test the shellscript for setup with [Bash Automated Testing System (bats)](https://github.com/bats-core/bats-core).
The scripts for the unit test can be found under [`./tests`](https://github.com/nrtkKodama/dotfiles/tree/master/tests/install) directory.

### 📦 Continuously monitor code coverage with Codecov [![codecov](https://codecov.io/gh/nrtkKodama/dotfiles/branch/master/graph/badge.svg?token=4VUJWKGAR7)](https://codecov.io/gh/nrtkKodama/dotfiles)

The code coverage of the [`./install`](https://github.com/nrtkKodama/dotfiles/tree/master/install) scripts are continuously monitored at [app.codecov.io/gh/nrtkKodama/dotfiles](https://app.codecov.io/gh/nrtkKodama/dotfiles). The following Icicle graph represents the code coverage of the scripts:

[![](https://codecov.io/gh/nrtkKodama/dotfiles/branch/master/graphs/icicle.svg?token=4VUJWKGAR7)](https://app.codecov.io/gh/nrtkKodama/dotfiles)
...
- Download [`.tmux.conf.d/system/server.conf`](https://github.com/nrtkKodama/dotfiles/blob/master/home/dot_tmux.conf.d/system/server.conf) and deploy as `~/.tmux.conf`

```shell
wget -O ~/.tmux.conf https://raw.githubusercontent.com/nrtkKodama/dotfiles/main/home/dot_tmux.conf.d/system/server.conf
```

- Download [`.vimrc`](https://github.com/nrtkKodama/dotfiles/blob/master/home/dot_vimrc) and deploy to `~/.vimrc`

```shell
wget -O ~/.vimrc https://raw.githubusercontent.com/nrtkKodama/dotfiles/main/home/dot_vimrc
```
